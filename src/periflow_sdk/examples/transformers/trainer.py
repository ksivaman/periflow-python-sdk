...
@dataclass
class HFTrainStepOutput(TrainStepOutput):
    loss: float
...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        # Periflow only supports step-based checkpointing.
        assert args.save_strategy == IntervalStrategy.STEPS
        periflow_recover_step = init(total_train_steps=max_steps,
                                     save_interval=args.save_steps,
                                     save_dir=args.output_dir,
                                     local_rank=args.local_rank if args.local_rank >= 0 else 0)  # args.local_rank is -1 if non-distributed.
        add_modules_and_recover({'model': self.model})
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            if periflow_recover_step == 0:
                self.state = TrainerState.load_from_json(os.path.join(resume_from_checkpoint, "trainer_state.json"))
                set_current_iteration(self.state.global_step)
            elif periflow_recover_step > 0:
                self.state.global_step = periflow_recover_step
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss += self.training_step(model, inputs).loss
                else:
                    tr_loss += self.training_step(model, inputs).loss
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        if not no_load and os.path.isfile(os.path.join(checkpoint, "optimizer.pt")) and os.path.isfile(
            ...
            if is_torch_tpu_available():
                ...
                if self.use_amp and os.path.isfile(os.path.join(checkpoint, "scaler.pt")):
                    ...
                    add_modules_and_recover({'scaler': self.scaler})
        modules = {'optimizer': self.optimizer, 'lr_scheduler': self.lr_scheduler}
        if self.use_amp:
            modules['scaler'] = self.scaler
        add_modules_and_recover(modules)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        return HFTrainStepOutput(loss=loss.detach())
